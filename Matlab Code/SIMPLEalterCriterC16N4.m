...
%                if(currPhase{c}(jumping{c})>1)
%                    currPhase{c}(jumping{c}) = 1;
%                end
%                if(currPhase{c}(jumping{c})<0)
%                    currPhase{c}(jumping{c}) = 0;
%                end
%                if(nesterov_Phase{c}(jumping{c})>1)
%                    nesterov_Phase{c}(jumping{c}) = 1;
%                end
%                if(nesterov_Phase{c}(jumping{c})<0)
%                    nesterov_Phase{c}(jumping{c}) = 0;
%                end
...
%         end
        ...
        objFunction(c,1) = 0.5*(norm(D*(sortedPhase{c}')-uVec+eVec))^2;
        if ( (objFunction(c,1) > desyncThreshold) || (desync_lost_frames(c) >= convergenceLoss) )
            convergedIterations{c} = 0;
            desync_lost_frames(c) = 0;
            itsConverged{c} = 0;
        end
...
umax=200000;
itsmax = 700; % x 100ms periods = 1 minute
...
max_value_timing_error=0.0001;
min_value_timing_error=-0.0001;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% figure(6)
% plot(1:itsmax, costFunctionEvolution(1,:))
